...
import { isFunctionBackedProClubWebAvailable } from "../src/config/runtimeCapabilities.ts";
...
test("runtime spoof fields cannot replace the bound club or actor", async () => {
  const hostileDraft = { ...baseDraft(), clubId: "attacker-club", authorUid: "attacker-user" } as ProClubWeeklyTrainingFreshDraftInput;
  ...
  await saveProClubWeeklyTrainingFreshDraft(
    ...
    async (submitted) => {
      captured = submitted;
      return { data: { status: "COMPLETED", clubId: "club-a", planId: "server-plan", documentCount: 3, createdAt: "2026-09-09T16:00:00.000Z" } };
    },
  );
...
test("runtime Technical Director note smuggling fails before transport", async () => {
  const hostileDraft = { ...baseDraft(), technicalDirectorNote: "smuggled client authority field" } as ProClubWeeklyTrainingFreshDraftInput;
  let called = false;
  await assert.rejects(
    saveProClubWeeklyTrainingFreshDraft(
      { clubId: "club-a", actorUid: "coach-1", draft: hostileDraft },
      async () => { called = true; return { data: null }; },
    ),
...
test("non-string Technical Director note smuggling also fails before transport", async () => {
  const hostileDraft = { ...baseDraft(), technicalDirectorNote: { injected: true } } as ProClubWeeklyTrainingFreshDraftInput;
  let called = false;
  await assert.rejects(
    saveProClubWeeklyTrainingFreshDraft(
      { clubId: "club-a", actorUid: "coach-1", draft: hostileDraft },
...
test("client and composer contain no browser Firestore Weekly Training write fallback", async () => {
  ...
  for (const forbidden of [/firebase\/firestore/, /\bsetDoc\b/, /\baddDoc\b/, /\bupdateDoc\b/, /\bdeleteDoc\b/, /\bwriteBatch\b/, /\brunTransaction\b/]) {
