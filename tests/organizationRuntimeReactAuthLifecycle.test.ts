...
test(
  ...
  async (t) => {
    ...
    await t.test("provider composes the approved runtime lifecycle API succession", () => {
      ...
      assert.doesNotMatch(
        contextSource,
        /\bclearOrganizationRuntime\b/,
      );
      assert.match(
        contextSource,
        /const selected = selectOrganization\(\s*current\.runtimeState,\s*"PRO_CLUB",\s*organizationId,\s*\);/s,
      );
      assert.match(
        contextSource,
      ...
      assert.match(
        contextSource,
        /applyOrganizationResolution\(current\.runtimeState, bridgeResult\.runtimeResult\)/,
      );
    });
