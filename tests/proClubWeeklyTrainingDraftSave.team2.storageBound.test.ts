...
function fullDraft(drillReference: string) {
  return {
    clubId: "club-a",
    authorUid: "coach-1",
...
function clientDraft(drillReference: string) {
  const { clubId: _clubId, authorUid: _authorUid, ...draft } = fullDraft(drillReference);
  return draft;
}
...
test("Team2: UI and trusted server independently enforce byte length rather than maxLength alone", async () => {
  ...
  assert.match(composer, /proClubTrainingUtf8ByteLength\(next\)/);
