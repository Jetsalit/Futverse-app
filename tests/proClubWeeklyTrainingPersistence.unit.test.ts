...
describe("Pro Club Weekly Training persistence foundation", () => {
  it("builds exact normalized Pro Club-only canonical paths", () => {
    assert.equal(proClubWeeklyTrainingPlanPath("club-1", "plan-1"), "proClubs/club-1/weeklyTrainingPlans/plan-1");
    assert.equal(proClubWeeklyTrainingSessionPath("club-1", "plan-1", "2026-09-08-1600"), "proClubs/club-1/weeklyTrainingPlans/plan-1/sessions/2026-09-08-1600");
    assert.equal(proClubWeeklyTrainingBlockPath("club-1", "plan-1", "2026-09-08-1600", "block-01"), "proClubs/club-1/weeklyTrainingPlans/plan-1/sessions/2026-09-08-1600/blocks/block-01");
    assert.equal(proClubTechnicalGovernanceCurrentPath("club-1"), "proClubs/club-1/technicalGovernance/current");
  });
  ...
  it("rejects padded and path-like identifiers at every path level", () => {
    ...
    assert.equal(proClubWeeklyTrainingSessionPath("club-1", "plan-1", "sessions/s1"), null);
    assert.equal(proClubWeeklyTrainingBlockPath("club-1", "plan-1", "s1", " block-1 "), null);
  });

  it("normalizes a domain-valid DRAFT into integrity-bearing schema v2 documents", () => {
    const result = buildProClubWeeklyTrainingDraftWrite({ clubId: "club-1", planId: "plan-1", plan: validPlan() });
    assert.equal(result.state, "VALID");
    if (result.state !== "VALID") throw new Error("Expected valid persistence bundle");

    assert.equal(result.bundle.planPayload.schemaVersion, 2);
    assert.equal(result.bundle.planPayload.status, "DRAFT");
    assert.equal(result.bundle.planPayload.authorUid, "hc-1");
    assert.equal(result.bundle.planPayload.sessionCount, 1);
    ...
    assert.equal(session.payload.schemaVersion, 2);
    assert.equal(session.payload.orderIndex, 0);
    assert.equal(session.payload.blockCount, 1);
    ...
    assert.equal(session.blocks[0].payload.schemaVersion, 2);
    assert.equal(session.blocks[0].payload.orderIndex, 0);
    assert.deepEqual(session.blocks[0].payload.coachingPoints, ["Create the third-man option"]);
  });

  it("fails closed when the plan tenant does not match the path tenant", () => {
    assert.deepEqual(buildProClubWeeklyTrainingDraftWrite({ clubId: "club-2", planId: "plan-1", plan: validPlan() }), { state: "INVALID", reason: "INVALID_PLAN" });
  });

  it("fails closed before normalization when deep plan validation fails", () => {
    const plan = validPlan();
    plan.sessions[0].blocks[0].coachingPoints = [];
    assert.deepEqual(buildProClubWeeklyTrainingDraftWrite({ clubId: "club-1", planId: "plan-1", plan }), { state: "INVALID", reason: "INVALID_PLAN" });
  });
