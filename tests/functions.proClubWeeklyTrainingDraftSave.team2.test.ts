...
const PROJECT_ID = "demo-futverse-weekly-training-server-team2-r2";
const CLUB_ID = "club-a";
const ACTOR_UID = "hc-a";
const TD_UID = "td-a";
const app = initializeApp({ projectId: PROJECT_ID }, "weekly-training-team2-r2");
...
before(async () => { assert.ok(process.env.FIRESTORE_EMULATOR_HOST); await clear(); });
beforeEach(async () => { await clear(); await seed(); });
after(async () => { await clear(); await deleteApp(app); });

test("payload author spoof is ignored and text normalizes like the domain", async () => {
  ...
  assert.equal(plan.data()?.squadLabel, "First Team");
  assert.equal(plan.data()?.mainObjective, "Canonical plan");
});
