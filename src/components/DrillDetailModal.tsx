...
import { X, Clock, User, Tag, Edit2, Trash2, CalendarCheck, LayoutGrid, Users, FileText } from 'lucide-react';
...
export default function DrillDetailModal({ 
  drill, isOpen, onClose, currentUser, onEdit, onDelete 
      ...
      <div className="relative w-full max-w-4xl bg-white rounded-2xl shadow-2xl flex flex-col max-h-full animate-in zoom-in-95 duration-200">
        ...
        <div className="flex-1 overflow-y-auto p-6 sm:p-10">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
             <div>
                <div className="text-[12px] font-bold text-slate-400 uppercase tracking-widest mb-1">TRAINING LOGBOOK</div>
                <h1 className="text-3xl sm:text-4xl font-black text-slate-800 tracking-tight mb-3">{drill.title}</h1>
             ...
             <div className="text-right">
                <div className="text-xs font-bold text-slate-500">วันที่ฝึกซ้อม</div>
                <div className="text-lg font-bold text-slate-800">{drill.date || 'ไม่ได้ระบุ'}</div>
             </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
             <div>
                ...
                <div className="font-medium text-slate-800 text-sm">{drill.duration || '-'}</div>
             </div>
             <div>
                <div className="flex items-center gap-2 text-slate-500 mb-1">
                   <Users size={16} />
                ...
                <div className="font-medium text-slate-800 text-sm">{drill.ageGroup || '-'}</div>
             </div>
             <div>
                <div className="flex items-center gap-2 text-slate-500 mb-1">
                   <FileText size={16} />
                ...
                <div className="font-medium text-slate-800 text-sm">{drill.created_by}</div>
             </div>
             <div>
                <div className="flex items-center gap-2 text-slate-500 mb-1">
                   <span className="text-xs font-bold">Phase</span>
                </div>
                <div className="font-medium text-slate-800 text-sm">{drill.phase || '-'}</div>
             </div>
          </div>

          {/* Training Method */}
          <div className="bg-slate-50 border border-slate-100 rounded-xl p-5 mb-4">
             <h3 className="font-bold text-slate-800 mb-2">วิธีการฝึก (Training Method)</h3>
             <p className="text-slate-600 text-sm whitespace-pre-wrap">
               {drill.trainingMethod || <span className="text-slate-400 italic">ไม่มีข้อมูล</span>}
             </p>
          </div>

          {/* Coaching Points */}
          <div className="bg-slate-50 border border-slate-100 rounded-xl p-5 mb-8">
             <h3 className="font-bold text-slate-800 mb-2">จุดโค้ชชิ่ง (Coaching Points)</h3>
             <p className="text-slate-600 text-sm whitespace-pre-wrap">
               {drill.coachingPoints || <span className="text-slate-400 italic">ไม่มีข้อมูล</span>}
             </p>
          </div>

          {/* Tactic Board Preview */}
          <div>
            <h3 className="font-bold text-slate-800 mb-4 text-lg">แผนการฝึกซ้อม (Tactic Board)</h3>
      ...
      {showDeleteConfirm && (
        <div className="absolute inset-0 z-[70] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl w-full max-w-sm shadow-2xl p-6 text-center animate-in zoom-in-95 duration-200">
            ...
            <h3 className="text-xl font-bold text-slate-900 mb-2">ยืนยันการลบแบบฝึก</h3>
            <p className="text-slate-500 text-sm mb-6 leading-relaxed">คุณต้องการลบ <b>"{drill.title}"</b> ใช่หรือไม่? ข้อมูลที่ถูกลบจะไม่สามารถกู้คืนได้</p>
