...
const FORMATIONS: Record<string, Position[]> = {
  '4-4-2': [
    { x: 50, y: 90, label: 'GK' },
    { x: 15, y: 70, label: 'LB' }, { x: 35, y: 70, label: 'CB' }, { x: 65, y: 70, label: 'CB' }, { x: 85, y: 70, label: 'RB' },
    { x: 15, y: 45, label: 'LM' }, { x: 35, y: 45, label: 'CM' }, { x: 65, y: 45, label: 'CM' }, { x: 85, y: 45, label: 'RM' },
    { x: 35, y: 20, label: 'ST' }, { x: 65, y: 20, label: 'ST' }
  ],
  '4-3-3': [
    { x: 50, y: 90, label: 'GK' },
    { x: 15, y: 70, label: 'LB' }, { x: 35, y: 70, label: 'CB' }, { x: 65, y: 70, label: 'CB' }, { x: 85, y: 70, label: 'RB' },
  ...
  '3-5-2': [
    { x: 50, y: 90, label: 'GK' },
    { x: 25, y: 75, label: 'CB' }, { x: 50, y: 70, label: 'CB' }, { x: 75, y: 75, label: 'CB' },
    { x: 15, y: 45, label: 'LWB' }, { x: 35, y: 50, label: 'CM' }, { x: 50, y: 40, label: 'CAM' }, { x: 65, y: 50, label: 'CM' }, { x: 85, y: 45, label: 'RWB' },
    { x: 35, y: 20, label: 'ST' }, { x: 65, y: 20, label: 'ST' }
  ]
};
...
const MOCK_SQUAD = [
  ...
  { id: '14', name: 'Pathompol Charoenrattanapirom', number: 14, position: 'RW' },
...
export default function StartingXIBuilder({ onBack }: { onBack: () => void }) {
  ...
  const filteredSquad = MOCK_SQUAD.filter(p => 
    ...
    <div className="w-full max-w-6xl mx-auto pb-10 flex flex-col h-[calc(100vh-4rem)]">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4 shrink-0">
        <div>
          ...
          <h1 className="text-3xl font-black text-slate-800 tracking-tight">Starting XI & Tactics</h1>
          <p className="text-sm font-medium text-slate-500 mt-1">Design your match setup and player assignments</p>
        </div>
        <div className="flex items-center gap-3 bg-white p-2 rounded-xl shadow-sm border border-slate-200">
           <label className="text-xs font-bold text-slate-400 uppercase tracking-widest pl-2">Formation</label>
           <select 
              value={formation} 
      ...
      <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 flex-1 min-h-0">
        ...
        <div className="w-full lg:w-2/3 overflow-x-auto rounded-3xl shadow-xl border-[6px] border-white flex-shrink-0 bg-slate-100 hidden-scrollbar">
           ...
           <div className="absolute inset-0 opacity-20 pointer-events-none" style={{ backgroundImage: 'repeating-linear-gradient(0deg, transparent, transparent 50px, rgba(0,0,0,0.1) 50px, rgba(0,0,0,0.1) 100px)' }}></div>
                 ...
                 <div className="relative">
                   <button 
                     ...
                     className={`w-8 h-8 md:w-12 md:h-12 rounded-full border-2 flex items-center justify-center font-black text-sm md:text-lg transition-transform ${
                       isSelected ? 'ring-4 ring-amber-400 scale-110 shadow-lg' : 'hover:scale-110 shadow-md'
                     } ${
                       player ? 'bg-white text-indigo-800 border-indigo-200' : 'bg-black/30 border-white/50 text-white border-dashed'
                     }`}
                   >
                   ...
                   {player && (
                     ...
                     >
                        <X size={10} strokeWidth={3} className="md:w-3 md:h-3" />
                     </button>
                   )}
                 </div>
        ...
        <div className="w-full lg:w-1/3 h-[500px] lg:h-full flex flex-col bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden shrink-0">
          <div className="p-5 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
            ...
            <div className={`text-xs font-bold px-3 py-1 rounded-full ${selectedCount === 11 ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-600'}`}>
          ...
          <div className="p-4 border-b border-slate-100">
             ...
             {activeSlot !== null && (
               <div className="mt-3 p-3 bg-indigo-50 border border-indigo-100 rounded-xl flex items-center justify-between animate-pulse">
                  <span className="text-xs font-bold text-indigo-800">Select a player for this position</span>
                  <button onClick={() => setActiveSlot(null)} className="text-indigo-400 hover:text-indigo-600"><X size={16}/></button>
               </div>
             )}
          </div>
                ...
                <button
                  ...
                  className={`w-full flex items-center justify-between p-3 mb-1.5 rounded-xl border transition-all text-left group ${
                    isAssigned 
                      ...
                      : activeSlot !== null ? 'bg-white border-slate-200 hover:border-indigo-400 hover:shadow-sm cursor-pointer' : 'bg-white border-transparent cursor-default'
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <div className={`w-9 h-9 rounded-full flex items-center justify-center font-black text-sm transition-colors ${
                      isAssigned ? 'bg-slate-200 text-slate-500' : 'bg-indigo-100 text-indigo-700 group-hover:bg-indigo-600 group-hover:text-white'
                    }`}>
                      {player.number}
                    </div>
                    <div>
                      <div className="font-bold text-sm text-slate-800">{player.name}</div>
                      <div className="text-[10px] font-black text-slate-400">{player.position}</div>
                    </div>
                  </div>
                  {isAssigned ? (
                    <CheckCircle2 size={18} strokeWidth={2.5} className="text-emerald-500" />
                  ) : activeSlot !== null ? (
