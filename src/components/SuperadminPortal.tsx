...
export default function SuperadminPortal({ onBack }: { onBack: () => void }) {
  ...
  const [activeTab, setActiveTab] = useState<"users" | "approvals" | "staff">(
    "users",
  );
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm font-medium text-slate-500">
              Manage users and global system settings.
            </p>
          </div>
        </div>
      </div>
                  ...
                  .map((user) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-right">
                        ...
                        >
                          <span className="text-base leading-none">🎭</span> Act
                          As
                        </button>
                      </td>
                    </tr>
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm font-bold text-slate-700">
                          {user.paymentDetails?.date}
                        </div>
                        <div className="text-xs text-slate-500">
                          at {user.paymentDetails?.time}
                        </div>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <button
        ...
        {activeTab === "staff" && (
          <div className="overflow-x-auto p-4 max-w-4xl">
            ...
            <div className="space-y-4">
              {users
                .filter((u) => u.role === "DATA_ADMIN")
                .map((staff) => (
                  <div
                    key={staff.id}
                    className="p-4 border border-slate-200 rounded-xl bg-white flex items-center justify-between shadow-sm"
                  >
                    <div>
                      <div className="font-bold text-slate-800 text-sm">
                        {staff.name}
          ...
          <div className="bg-white rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto">
              <p className="text-sm text-slate-500 mb-4">
                Select clients (Coaches/Academies) to assign to{" "}
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto border border-slate-200 rounded-xl p-2 bg-slate-50">
                {users
                  .filter((u) => u.role === "COACH" || u.role === "USER")
                  .map((client) => {
                    const isAssigned = selectedStaff.assignedClients?.includes(
                      client.id!,
                    );
                    return (
                      <label
                        key={client.id}
                        className="flex items-center gap-3 p-3 bg-white border border-slate-100 rounded-lg cursor-pointer hover:border-slate-300 transition-colors"
                      >
                        <input
                          type="checkbox"
                          ...
                          onChange={(e) => {
                            const newAssigned = e.target.checked
                              ? [
                                  ...(selectedStaff.assignedClients || []),
                                  client.id!,
                                ]
                              : (selectedStaff.assignedClients || []).filter(
                                  (id) => id !== client.id,
                                );

                            const updatedStaff = {
                              ...selectedStaff,
                              assignedClients: newAssigned,
                            };
                            setSelectedStaff(updatedStaff);
                            setUsers(
                              users.map((u) =>
                                u.id === selectedStaff.id ? updatedStaff : u,
                              ),
                            );
                          }}
          ...
          <div className="bg-white rounded-3xl w-full max-w-xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto">
              ...
              <div className="w-full h-80 bg-slate-100 rounded-xl border border-slate-200 overflow-hidden mb-6 flex justify-center items-center">
                <img
                  src={selectedSlipUser.paymentDetails?.slipUrl || undefined}
                  alt="Slip"
