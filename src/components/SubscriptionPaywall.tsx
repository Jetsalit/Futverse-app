...
export default function SubscriptionPaywall() {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      setTimeout(() => {
        setSlipUrl(
          "https://images.unsplash.com/photo-1620714223084-8fcacc6dfd8d?auto=format&fit=crop&q=80&w=300",
        ); // Mock image URL
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="bg-slate-50 rounded-2xl p-6 text-center border border-slate-100">
                ...
                <p className="text-xs text-slate-500 font-medium">
                  ชื่อบัญชี: บจก. ฟุตเวิร์ส อคาเดมี
                </p>
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-1.5">
                    แนบสลิปโอนเงิน <span className="text-rose-500">*</span>
                  </label>
                  {!slipUrl ? (
                    ...
                    <div className="relative w-full h-32 rounded-xl overflow-hidden group border border-slate-200">
                      ...
                      <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
                        <button
                          type="button"
                          onClick={() => setSlipUrl("")}
                          className="bg-white/20 p-2 rounded-full text-white hover:bg-rose-500 transition-colors"
                        >
                          <X size={20} />
                        </button>
                      </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-1.5">
                      เวลาที่โอน <span className="text-rose-500">*</span>
                    </label>
                    <input
                      type="time"
                      required
