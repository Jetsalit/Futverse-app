...
import { Save, User, ShieldAlert, Activity, ChevronLeft, ClipboardList, Database, Check } from 'lucide-react';
...
const PlayerTestRow = memo(({ player, rowData, onChange }: { player: any, rowData: any, onChange: (id: string, field: string, value: string) => void }) => {
...
function FitnessTestingGrid({ 
  ...
  const handleRowChange = useCallback((playerId: string, field: string, value: string) => {
    setTestData(prev => {
  ...
  const handleSaveAllResults = () => {
    ...
    const payloadToSave = players.map(player => {
    ...
    setTimeout(() => {
      setIsSaving(false);
      console.log('--- Payload Prepared for Upsert (Supabase) ---', payloadToSave);
  ...
  return (
    <>
      <div className="px-6 py-4 border-b border-slate-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-emerald-50 text-emerald-600 rounded-lg shrink-0">⚡</div>
          <div>
            <h2 className="font-bold text-sm sm:text-base">Squad Fitness Testing Bulk Entry</h2>
            <div className="text-xs text-slate-400 font-medium whitespace-nowrap">Input auto-calculates secondary metrics</div>
          </div>
        </div>
        
        <div className="flex items-center gap-3 w-full sm:w-auto">
          {saveStatus === 'success' && (
            <span className="text-xs font-bold text-emerald-600 flex items-center gap-1"><Check size={14}/> Saved to Cloud</span>
          )}
          {saveStatus === 'offline_queued' && (
            <span className="text-xs font-bold text-amber-600 flex items-center gap-1"><Database size={14}/> Saved Offline</span>
          )}
        ...
        <table className="w-full text-left border-collapse min-w-[800px]">
          <thead>
            <tr className="bg-slate-50 text-[11px] uppercase tracking-wider text-slate-500 font-bold">
              <th className="px-6 py-3 border-b sticky left-0 bg-slate-50 z-10 w-72 shadow-[1px_0_0_#e2e8f0]">Player Info & Actions</th>
              {METRICS_CONFIG.map(m => (
                <th key={m.key} className="px-4 py-3 border-b text-center align-bottom min-w-[120px]">
                  {m.label} <span className="font-normal text-slate-400 block mt-0.5">({m.unit})</span>
                </th>
              ))}
            </tr>
          </thead>
...
export default function FitnessTesting({ onBack, teamName, isOnline = true, onOfflineSave }: { onBack: () => void, teamName?: string, isOnline?: boolean, onOfflineSave?: () => void }) {
  const [activeTab, setActiveTab] = useState<'entry' | 'report'>('entry');
  const [selectedPlayerId, setSelectedPlayerId] = useState<number>(MOCK_PLAYERS[0].id);
  
  // State from main wrapper for reports functionality and passing to grid
  const [testData, setTestData] = useState<Record<string, Record<string, string>>>({});
  const [saveStatus, setSaveStatus] = useState<null | 'success' | 'offline_queued'>(null);

  const getRadarData = (playerId: number) => {
    return METRICS_CONFIG.filter(m => m.key !== 'calculated_vo2max').map(m => {
      let val = 0;
      if (testData[playerId]?.[m.key]) {
        ...
        val = m.key.includes('speed') ? 4 + (playerId * 0.1) : 
              m.key === 'vertical_jump' ? 50 + (playerId * 2) : 
              m.key === 'yoyo_level' ? 14 + (playerId * 0.5) : 15 - (playerId * 0.2);
      }
  ...
  return (
    <div className="w-full flex-1 flex flex-col animate-in fade-in duration-300 relative">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-800 tracking-tight">Fitness Testing System</h1>
          <p className="text-[11px] text-slate-400 font-bold uppercase tracking-widest mt-1">Assessment Engine</p>
        </div>
      </div>
      ...
      <div className="flex border-b border-slate-200 mb-6 space-x-8">
        <button
          ...
          className={`pb-4 text-sm font-medium transition-colors relative flex items-center gap-2 ${
            activeTab === 'entry' ? 'text-emerald-600' : 'text-slate-500 hover:text-slate-800'
          }`}
        >
          <ClipboardList size={18} />
          Squad Entry Grid
          {activeTab === 'entry' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-emerald-600" />}
        </button>
        <button
          onClick={() => setActiveTab('report')}
          className={`pb-4 text-sm font-medium transition-colors relative flex items-center gap-2 ${
            activeTab === 'report' ? 'text-emerald-600' : 'text-slate-500 hover:text-slate-800'
          }`}
        >
          <Activity size={18} />
          Player Reports Overview
          {activeTab === 'report' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-emerald-600" />}
        </button>
      </div>
      ...
      {activeTab === 'report' && (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 animate-in fade-in duration-300">
          <div className="lg:col-span-1 bg-white rounded-2xl shadow-sm border border-slate-200 p-4 sticky top-6 h-fit">
            <h3 className="text-xs font-bold text-slate-400 uppercase mb-4 px-2">Squad Roster</h3>
            <div className="space-y-1.5">
              {MOCK_PLAYERS.map(player => (
                ...
                >
                  ...
                  <span className={`text-[10px] uppercase font-bold tracking-wider px-2 py-1 rounded ${selectedPlayerId === player.id ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-400'}`}>
                    {player.position}
                  </span>
                </button>
              ))}
            ...
            <div className="mt-6 p-4 bg-amber-50 rounded-xl border border-amber-100">
              <div className="flex gap-3 mb-2">
                ...
                <span className="font-semibold text-amber-800 text-sm">Injury Risk Analysis</span>
              </div>
              <div className="text-sm text-amber-700/90 leading-relaxed">
                High load detected in last 3 microcycles for selected position group. Recommend active recovery session.
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-3 space-y-6">
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
              <h3 className="text-xs font-bold text-slate-400 uppercase mb-6 border-b border-slate-100 pb-3">Performance Spider Chart</h3>
              <div className="h-[400px] w-full mt-4">
                <ResponsiveContainer width="100%" height="100%">
                  <RadarChart cx="50%" cy="50%" outerRadius="75%" data={getRadarData(selectedPlayerId)}>
                    ...
                    />
                    <Tooltip 
                      formatter={(value: any, name: any, props: any) => [props?.payload?.actualValue || value, "Value"]}
                      contentStyle={{ borderRadius: '12px', border: '1px solid #e2e8f0', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1), 0 2px 4px -2px rgb(0 0 0 / 0.1)', padding: '12px', fontSize: '12px', fontWeight: 'bold' }}
                    />
                  </RadarChart>
                </ResponsiveContainer>
              </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
              <h3 className="text-xs font-bold text-slate-400 uppercase mb-6 border-b border-slate-100 pb-3">Fitness Progression Timeline</h3>
              <div className="h-[300px] w-full mt-4">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={MOCK_HISTORIC_DATA} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                    <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fill: '#64748b', fontSize: 11, fontWeight: 'bold' }} dy={10} />
                    <YAxis yAxisId="left" orientation="left" stroke="#0ea5e9" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: '#64748b' }} domain={['dataMin - 0.5', 'dataMax + 0.5']} />
                    <YAxis yAxisId="right" orientation="right" stroke="#10b981" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: '#64748b' }} domain={['dataMin - 0.5', 'dataMax + 0.5']} />
                    <Tooltip 
                      contentStyle={{ borderRadius: '12px', border: '1px solid #e2e8f0', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)', padding: '12px', fontSize: '12px', fontWeight: 'bold' }}
                    />
                    <Legend iconType="circle" wrapperStyle={{ paddingTop: '20px', fontSize: '11px', fontWeight: 'bold', color: '#64748b' }} />
                    <Line yAxisId="right" type="monotone" name="Yo-Yo Level" dataKey="yoyo_level" stroke="#10b981" strokeWidth={3} dot={{ r: 5, strokeWidth: 2, fill: '#fff' }} activeDot={{ r: 7 }} />
                    <Line yAxisId="left" type="monotone" name="30m Sprint (sec)" dataKey="speed_30m" stroke="#0ea5e9" strokeWidth={3} dot={{ r: 5, strokeWidth: 2, fill: '#fff' }} activeDot={{ r: 7 }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
