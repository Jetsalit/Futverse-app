...
const RedTeamIcon = () => (
  <div className="w-3.5 h-3.5 bg-red-500 rounded-full border border-red-700 shadow-sm" />
);
const BlueTeamIcon = () => (
  <div className="w-3.5 h-3.5 bg-blue-500 rounded-full border border-blue-700 shadow-sm" />
);
const BallIcon = () => (
...
const ConeIcon = () => (
  <div className="w-0 h-0 border-l-[6px] border-l-transparent border-r-[6px] border-r-transparent border-b-[12px] border-b-orange-500 drop-shadow-sm" />
);
const GoalIcon = () => (
  <div className="w-4 h-3 flex items-end justify-center border-t-[2px] border-x-[2px] border-slate-500 rounded-t-[2px]" />
);
const HurdleIcon = () => (
  <div className="w-5 h-2.5 border-t-[3px] border-x-[3px] border-yellow-400" />
);
const LadderIcon = () => (
...
const DashedLineIcon = () => (
  <div className="w-4 border-b-[3px] border-dashed border-current" />
);
const CurveLineIcon = () => (
  <svg
    width="18"
            ...
            <>
              ...
              <div className="bg-slate-100 border-b border-slate-200 p-3 sm:p-4 flex flex-col gap-3 shrink-0 relative z-10">
                ...
                <div className="flex items-center justify-between gap-4">
                  ...
                  <button
                    onClick={handleUndo}
                    className="flex items-center gap-1.5 px-3 py-1.5 text-slate-600 hover:bg-slate-200 bg-white border border-slate-200 rounded-lg shadow-sm whitespace-nowrap shrink-0 transition-colors"
                  >
                    <Undo2 size={16} />
                ...
                <div className="flex items-center gap-4 flex-wrap">
                  <button
                    onClick={handleClear}
                    className="flex items-center gap-1.5 px-3 py-1.5 text-rose-600 hover:bg-rose-50 bg-white border border-rose-200 rounded-lg shadow-sm whitespace-nowrap shrink-0 transition-colors"
                  >
                    <Trash2 size={16} />
                  ...
                  <div className="flex items-center gap-1.5 bg-white p-1 rounded-lg border border-slate-200 shadow-sm overflow-x-auto">
                    <span className="text-xs font-bold text-slate-500 ml-2 mr-1">
                      วาดเส้น:
                    </span>
                    ...
                    {DRAWING_TOOLS.map((tool) => {
                      const Icon = tool.icon;
                      const isActive =
                        activeLineTool === tool.id && activeTool === "draw";
                      return (
                        <button
                          key={tool.id}
                          onClick={() => {
              ...
              <div className="flex-1 relative w-full overflow-auto bg-slate-50 flex flex-col p-2 lg:p-6">
                <div
                  className={`relative bg-white ring-1 ring-slate-300 shadow-sm overflow-hidden select-none shrink-0 ${activeTool !== 'pan' ? 'touch-none' : ''} ${
                    fieldType === "full" ? "aspect-[1.54/1] w-full min-w-[600px] md:min-w-0" : fieldType === "small" ? "aspect-[2/1] w-full min-w-[600px] md:min-w-0" : "aspect-[1/1.25] w-full max-w-[500px]"
                  }`}
                  style={{
                     maxHeight: "100%",
                ...
                <div className="absolute inset-4 lg:inset-8 ring-[1.5px] ring-slate-800 pointer-events-none z-0">
                  {fieldType === "full" ? (
                    ...
                    <>
                      ...
                      {/* Center circle */}
                      <div className="absolute top-1/2 left-1/2 w-[15%] aspect-square border-[1.5px] border-slate-800 rounded-full -translate-x-1/2 -translate-y-1/2"></div>
                      
                      {/* Center mark */}
                      <div className="absolute top-1/2 left-1/2 w-1.5 h-1.5 bg-slate-800 rounded-full -translate-x-1/2 -translate-y-1/2"></div>

                      {/* Left Penalty Area (semi-circle) */}
                      <div className="absolute top-1/2 left-0 w-[15%] h-[60%] border-[1.5px] border-slate-800 -translate-y-1/2 border-l-0 bg-white" style={{ borderTopRightRadius: "9999px", borderBottomRightRadius: "9999px" }}></div>
                      ...
                      <div className="absolute top-1/2 right-0 w-[15%] h-[60%] border-[1.5px] border-slate-800 -translate-y-1/2 border-r-0 bg-white z-10" style={{ borderTopLeftRadius: "9999px", borderBottomLeftRadius: "9999px" }}></div>
                      
                      {/* Right Penalty Mark */}
                      <div className="absolute top-1/2 right-[12%] w-1 h-1 bg-slate-800 rounded-full -translate-y-1/2 z-20"></div>

                      {/* Goal nets */}
                      <div className="absolute top-1/2 left-0 w-[2%] h-[15%] border-[1.5px] border-slate-800 -translate-y-1/2 -translate-x-full border-r-0"></div>
                      <div className="absolute top-1/2 right-0 w-[2%] h-[15%] border-[1.5px] border-slate-800 -translate-y-1/2 translate-x-full border-l-0"></div>
              ...
              {uploadedImage ? (
                <div className="relative w-full max-w-2xl mx-auto flex flex-col items-center justify-center h-full">
                  <img
                    src={uploadedImage}
                    alt="Uploaded Drill"
                    className="max-h-full max-w-full rounded-lg shadow-sm border border-slate-200 object-contain"
                  />
                  <button
                ...
                <label className="w-full max-w-lg aspect-video flex flex-col items-center justify-center p-8 border-2 border-dashed border-slate-300 rounded-2xl bg-white hover:bg-slate-50 hover:border-blue-400 transition-colors cursor-pointer text-center group shadow-sm">
                  ...
                  <h3 className="text-lg font-bold text-slate-800 mb-2">
                    อัปโหลดรูปภาพ / ถ่ายรูป
                  </h3>
                  <p className="text-sm text-slate-500 max-w-sm mx-auto">
                    คลิกเพื่อเลือกไฟล์รูปภาพจากอุปกรณ์
                    หรือใช้กล้องถ่ายรูปสมุดโน้ตวาดมือ
                  </p>
                  <input
                    type="file"
                    accept="image/*"
          ...
          <div className="p-4 sm:p-5 space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none"
                placeholder="เช่น Rondo 4v2"
              />
            </div>
            <div>
              <label className="block text-[11px] font-bold text-slate-500 mb-1.5 uppercase">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  type="text"
                  value={saveForm.phase}
                  onChange={(e) =>
                    setSaveForm({ ...saveForm, phase: e.target.value })
                  }
                  className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
                  placeholder="Preparatory - General"
                />
              </div>
              <div>
                <label className="block text-[11px] font-bold text-slate-500 mb-1.5 uppercase">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSaveForm({ ...saveForm, duration: e.target.value })
                  }
                  className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
                  placeholder="60 นาที"
                />
            ...
            <div>
              ...
              <input
                type="text"
                value={saveForm.ageGroup}
                onChange={(e) =>
                  setSaveForm({ ...saveForm, ageGroup: e.target.value })
                }
                className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
                placeholder="รุ่น U12"
              />
            </div>
            <div>
              <label className="block text-[11px] font-bold text-slate-500 mb-1.5 uppercase">
              ...
              <textarea
                value={saveForm.trainingMethod}
                onChange={(e) =>
                  setSaveForm({ ...saveForm, trainingMethod: e.target.value })
                }
                className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none min-h-[80px] resize-y"
                placeholder="อธิบายวิธีการซ้อม..."
              />
            </div>
            <div>
              <label className="block text-[11px] font-bold text-slate-500 mb-1.5 uppercase">
              ...
              <textarea
                value={saveForm.coachingPoints}
                onChange={(e) =>
                  setSaveForm({ ...saveForm, coachingPoints: e.target.value })
                }
                className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none min-h-[80px] resize-y"
                placeholder="พืัน, การเคลื่อนที่, ความแม่นยำ..."
              />
            </div>
