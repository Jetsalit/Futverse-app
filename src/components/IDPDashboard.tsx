...
import { ChevronLeft, Search, Filter, TrendingUp, AlertCircle, CheckCircle2 } from 'lucide-react';
...
const MOCK_IDP_SUMMARY: IDPPlayerSummary[] = [
  ...
  },
  {
    ...
    name: 'Ekanit Panya',
    position: 'Winger',
    avatarUrl: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Ekanit&backgroundColor=ea580c',
    team: 'Senior Team',
    overallProgress: 95,
  ...
  },
  {
    ...
    position: 'Forward',
    avatarUrl: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Suphanat&backgroundColor=0d9488',
    team: 'U-23 Squad',
    overallProgress: 75,
...
export default function IDPDashboard({ onBack, onNavigateToPlayer }: { onBack: () => void, onNavigateToPlayer?: (id: string) => void }) {
  const [searchTerm, setSearchTerm] = useState('');
  
  const filteredPlayers = MOCK_IDP_SUMMARY.filter(p => 
  ...
  const getStatusColor = (status: string) => {
    switch(status) {
      ...
      case 'Exceeding': return 'text-indigo-600 bg-indigo-50 border-indigo-200';
  ...
  const getStatusIcon = (status: string) => {
    switch(status) {
      case 'On Track': return <CheckCircle2 size={14} />;
      case 'Needs Attention': return <AlertCircle size={14} />;
      case 'Exceeding': return <TrendingUp size={14} />;
      default: return null;
    }
  };

  const averageProgress = Math.round(MOCK_IDP_SUMMARY.reduce((acc, curr) => acc + curr.overallProgress, 0) / MOCK_IDP_SUMMARY.length);
    ...
    <div className="flex flex-col h-full space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-black text-slate-800 tracking-tight uppercase">IDP Dashboard Overview</h1>
            <p className="text-[11px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">FIFA & JFA Standard Tracking</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Squad IDP Active</p>
            <h3 className="text-3xl font-black text-slate-800">{MOCK_IDP_SUMMARY.length}</h3>
          </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Avg Overall Form</p>
            <h3 className="text-3xl font-black text-emerald-600">{averageProgress}%</h3>
          </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            ...
            <h3 className="text-3xl font-black text-rose-600">
              {MOCK_IDP_SUMMARY.filter(p => p.status === 'Needs Attention').length}
            </h3>
          </div>
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                ...
                <th className="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Overall Progress</th>
                <th className="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Short-term</th>
                <th className="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Long-term</th>
                ...
                <tr key={player.id} className="hover:bg-slate-50 transition-colors group">
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      <img src={player.avatarUrl || `https://api.dicebear.com/7.x/avataaars/svg?seed=${player.name}`} alt={player.name} className="w-10 h-10 rounded-full border border-slate-200 bg-slate-100 object-cover" />
                      <div>
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs font-bold text-slate-700">{player.overallProgress}%</span>
                    </div>
                  </td>
                  <td className="p-4">
                  ...
                  <td className="p-4">
                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border ${getStatusColor(player.status)}`}>
                      {getStatusIcon(player.status)}
                      {player.status}
                    </span>
                  </td>
                  <td className="p-4">
                    <span className="text-xs text-slate-500 font-medium">{player.lastUpdated}</span>
                  </td>
