...
import { ArrowLeft, Download, Share2, Play, Calendar, MapPin, Award } from 'lucide-react';
import { ProPlayer } from '../types/ProPlayer';
import { Radar, RadarChart, PolarGrid, PolarAngleAxis, PolarRadiusAxis, ResponsiveContainer } from 'recharts';
import IDPProfile from './IDPProfile';

export default function ProPlayerCV({ player, onBack }: { player: ProPlayer, onBack: () => void }) {
  const [activeTab, setActiveTab] = useState<'profile' | 'idp'>('profile');
  
  const radarData = [
    { subject: 'Technical', A: player?.attributes?.technical || 0, fullMark: 100 },
    { subject: 'Tactical', A: player?.attributes?.tactical || 0, fullMark: 100 },
    { subject: 'Physical', A: player?.attributes?.physical || 0, fullMark: 100 },
    { subject: 'Mental', A: player?.attributes?.mental || 0, fullMark: 100 },
    { subject: 'Attacking', A: player?.attributes?.attacking || 0, fullMark: 100 },
    { subject: 'Defending', A: player?.attributes?.defending || 0, fullMark: 100 },
  ];
    ...
    <div className="flex flex-col h-full animate-in fade-in duration-300 relative bg-slate-50 overflow-y-auto">
      ...
      <div className="relative h-[60vh] min-h-[400px] w-full bg-slate-900 overflow-hidden shrink-0">
         ...
         <div className="absolute inset-0 opacity-40 mix-blend-overlay">
            <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="w-full h-full text-slate-800" fill="currentColor">
         ...
         <div className="absolute inset-0 flex flex-col md:flex-row items-end md:items-center justify-center pt-16 px-4 md:px-12 max-w-7xl mx-auto w-full">
            ...
            <div className="flex-1 text-left z-10 pb-8 md:pb-0 w-full">
               ...
               <h1 className="text-5xl md:text-7xl font-black text-white tracking-tighter uppercase leading-none drop-shadow-lg mb-2">
                  ...
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-emerald-400">{player.name.split(' ').slice(1).join(' ')}</span>
               </h1>
               
               <div className="flex flex-wrap items-center gap-6 mt-6">
                  <div className="text-white/80">
                     <div className="text-[10px] uppercase tracking-widest font-bold opacity-60">Position</div>
                     <div className="text-xl font-bold">{player.position}</div>
                  </div>
                  <div className="text-white/80 border-l border-white/20 pl-6">
                     <div className="text-[10px] uppercase tracking-widest font-bold opacity-60">Current Club</div>
                  ...
                  {player.marketValue && (
                     <div className="text-white/80 border-l border-white/20 pl-6 hidden sm:block">
                        <div className="text-[10px] uppercase tracking-widest font-bold opacity-60">Market Value</div>
                        <div className="text-xl font-bold text-emerald-400">{player.marketValue}</div>
                     </div>
                  )}
               </div>
            </div>
         ...
         {activeTab === 'profile' ? (
           <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
               <h3 className="text-lg font-black text-slate-800 uppercase tracking-tight mb-4">Player Profile</h3>
               <div className="space-y-4">
                  <div className="flex justify-between items-center py-2 border-b border-slate-100">
                     <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Age / Born</span>
                     <span className="text-sm font-bold text-slate-800">{new Date().getFullYear() - new Date(player.dob).getFullYear()} yrs ({player.dob})</span>
                  </div>
                  <div className="flex justify-between items-center py-2 border-b border-slate-100">
                     <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Height / Weight</span>
                     <span className="text-sm font-bold text-slate-800">{player.height} cm / {player.weight} kg</span>
                  </div>
                  <div className="flex justify-between items-center py-2 border-b border-slate-100">
                     <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Preferred Foot</span>
                     <span className="text-sm font-bold text-slate-800">{player.preferredFoot}</span>
                  </div>
                  <div className="flex justify-between items-center py-2">
                     <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Contract Expiry</span>
                     <span className="text-sm font-bold text-indigo-600">{player.contractExpiry}</span>
                  </div>
               </div>
            </div>
            ...
            <div className="bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-800 text-white relative overflow-hidden">
               ...
               <h3 className="text-sm font-black uppercase tracking-widest text-slate-400 mb-1">IDP Target Status</h3>
               <div className="flex items-end gap-3 mb-3">
                  <span className="text-4xl font-black text-emerald-400 leading-none">85%</span>
                  <span className="text-xs font-bold text-emerald-500 uppercase tracking-wider bg-emerald-500/10 px-2 py-1 rounded mb-1">Completion Rate</span>
               </div>
               <div className="w-full bg-slate-800 h-2 rounded-full overflow-hidden">
                  <div className="bg-gradient-to-r from-emerald-500 to-indigo-500 h-full rounded-full" style={{ width: '85%' }}></div>
               </div>
               <p className="text-[10px] text-slate-500 mt-3 font-medium uppercase tracking-wider">
                  <span className="text-slate-300 font-bold">17/20</span> development goals achieved for this season.
               </p>
            </div>
            ...
            {(player.phoneNumber || player.lineId || player.facebook) && (
               <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
                  <h3 className="text-lg font-black text-slate-800 uppercase tracking-tight mb-4">Contact Info</h3>
                  <div className="space-y-4">
                     {player.phoneNumber && (
                        <div className="flex justify-between items-center py-2 border-b border-slate-100">
                           <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Phone</span>
                           <span className="text-sm font-bold text-slate-800">{player.phoneNumber}</span>
                        </div>
                     )}
                     {player.lineId && (
                        <div className="flex justify-between items-center py-2 border-b border-slate-100">
                           <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Line ID</span>
                           <span className="text-sm font-bold text-slate-800">{player.lineId}</span>
                        </div>
                     )}
                     {player.facebook && (
                        <div className="flex justify-between items-center py-2">
                           <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Facebook</span>
                           <span className="text-sm font-bold text-slate-800">{player.facebook}</span>
                        </div>
                     )}
                  </div>
               </div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
               <h3 className="text-lg font-black text-slate-800 uppercase tracking-tight mb-2">Performance Attributes</h3>
               <p className="text-xs text-slate-500 mb-4">Scout assessment data mapping.</p>
               <div className="h-[250px] w-full -ml-4">
                  <ResponsiveContainer width="100%" height="100%">
                     <RadarChart cx="50%" cy="50%" outerRadius="70%" data={radarData}>
                        <PolarGrid stroke="#e2e8f0" />
                        <PolarAngleAxis dataKey="subject" tick={{ fill: '#64748b', fontSize: 10, fontWeight: 'bold' }} />
                        <Radar name="Attributes" dataKey="A" stroke="#4f46e5" fill="#4f46e5" fillOpacity={0.4} />
                     </RadarChart>
                  </ResponsiveContainer>
               </div>
            </div>
                  ...
                  {player.careerHistory?.map((history, idx) => (
                     <div key={idx} className="relative pl-6">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 mb-1">
                           <div className="font-black text-slate-800 text-lg">{history.club}</div>
                        ...
                        <div className="flex gap-4 mt-3">
                           <div className="bg-slate-50 px-3 py-2 rounded-xl border border-slate-100">
                              <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Appearances</div>
                              <div className="font-black text-slate-700">{history.apps}</div>
                           </div>
                           <div className="bg-slate-50 px-3 py-2 rounded-xl border border-slate-100">
                              <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Goals</div>
                              <div className="font-black text-emerald-600">{history.goals}</div>
                           </div>
                           <div className="bg-slate-50 px-3 py-2 rounded-xl border border-slate-100">
                              <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Assists</div>
                              <div className="font-black text-indigo-600">{history.assists}</div>
                           </div>
                        </div>
                     </div>
                  ))}
