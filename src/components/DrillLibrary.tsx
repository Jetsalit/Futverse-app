...
import { Plus, Users, User, LayoutGrid, Search, Edit2, Trash2, X } from 'lucide-react';
import DrillDetailModal from './DrillDetailModal';

export default function DrillLibrary({ onNavigate }: { onNavigate: (page: string) => void }) {
  const { myDrills, academyDrills, updateDrill, deleteDrill, currentUser } = useDrillDatabase();
  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 shrink-0">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-800 tracking-tight">คลังแบบฝึกซ้อม (Drill Library)</h1>
          <p className="text-[11px] text-slate-400 font-bold uppercase tracking-widest mt-1">Practice & Tactics DB</p>
        </div>
            ...
            {filteredDrills.map(drill => (
              ...
              >
                <div className="aspect-[4/3] bg-white border-b border-slate-100 relative flex items-center justify-center overflow-hidden group">
                  <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1 bg-white/80 backdrop-blur-sm p-1 rounded-lg z-20 shadow-sm border border-slate-100">
                    {drill.created_by === currentUser && (
                      <>
                        ...
                        <button 
                          onClick={(e) => { e.stopPropagation(); setDeletingDrillId(drill.id); }} 
                  ...
                  {drill.previewImage ? (
                    <img src={drill.previewImage} alt="Preview" className="absolute inset-0 w-full h-full object-contain pointer-events-none z-10 scale-[0.8]" />
                  ) : (
                ...
                <div className="p-4 flex flex-col flex-1">
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="font-bold text-slate-800 line-clamp-1">{drill.title}</h3>
                  </div>
                  <div className="mt-auto flex items-center justify-between pt-3">
                     ...
                     {activeTab === 'academy' && (
                        <span className="text-xs text-slate-400">By {drill.created_by}</span>
                     )}
                  </div>
                </div>
              </div>
      ...
      {editingDrill && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm" onClick={() => setEditingDrill(null)}></div>
          <div className="relative bg-white rounded-2xl w-full max-w-2xl shadow-xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <h2 className="text-lg font-bold text-slate-800">แก้ไขข้อมูลแบบฝึกซ้อม</h2>
              <button onClick={() => setEditingDrill(null)} className="text-slate-400 hover:text-slate-600 p-1 rounded-full hover:bg-slate-100 transition-colors">
                <X size={20} />
              </button>
            </div>
            <div className="p-6 overflow-y-auto space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2">
                  <label className="block text-xs font-bold text-slate-500 mb-1">ชื่อแผนการฝึก (Title)</label>
                  <input 
                    type="text" 
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-1">ระยะเวลา (Duration)</label>
                  <input 
                    type="text" 
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-1">กลุ่มผู้เล่น (Age Group)</label>
                  <input 
                    type="text" 
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-1">Phase</label>
                  <input 
                    type="text" 
                ...
                <div className="md:col-span-2">
                  <label className="block text-xs font-bold text-slate-500 mb-1">วิธีการฝึก (Training Method)</label>
                  <textarea 
                    value={editForm.trainingMethod}
                    onChange={e => setEditForm({...editForm, trainingMethod: e.target.value})}
                    rows={2}
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                  ></textarea>
                </div>
                <div className="md:col-span-2">
                  <label className="block text-xs font-bold text-slate-500 mb-1">จุดโค้ชชิ่ง (Coaching Points)</label>
                  <textarea 
                    value={editForm.coachingPoints}
                    onChange={e => setEditForm({...editForm, coachingPoints: e.target.value})}
                    rows={2}
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                  ></textarea>
                </div>
              ...
              <div className="flex items-center justify-between pt-2">
                <div>
                  <div className="font-bold text-sm text-slate-800">แชร์ลงคลังส่วนกลาง</div>
                  <div className="text-xs text-slate-500">ให้โค้ชคนอื่นเห็นแผนซ้อมนี้</div>
                </div>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input type="checkbox" className="sr-only peer" checked={editForm.is_shared} onChange={e => setEditForm({...editForm, is_shared: e.target.checked})} />
      ...
      {deletingDrillId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm" onClick={() => setDeletingDrillId(null)}></div>
          <div className="relative bg-white rounded-2xl w-full max-w-sm shadow-xl p-6 text-center animate-in zoom-in-95 duration-200">
            ...
            <p className="text-slate-500 text-sm mb-6">คุณต้องการลบข้อมูลแบบฝึกซ้อมนี้ใช่หรือไม่? ข้อมูลที่ถูกลบจะไม่สามารถกู้คืนได้</p>
            <div className="flex gap-3">
              <button onClick={() => setDeletingDrillId(null)} className="flex-1 px-4 py-2.5 rounded-xl font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 transition-colors">
                ยกเลิก
              </button>
              <button 
                onClick={() => {
